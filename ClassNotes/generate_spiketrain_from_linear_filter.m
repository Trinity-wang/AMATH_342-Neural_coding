...
T=1000 * 10^3; %total duration of spike train, in milliseconds
deltat=1;  %in ms
...
% MY CODE: Finding Spike Triggered Average
spikeI = find(spike_train);
lookback = 300;
stimSum = zeros(1,lookback + 1);
for index = spikeI
    stimSum = stimSum + stim_list(1, index - lookback:index);
end
STA = stimSum / length(spikeI);

figure(3);
xlabel('time preceeding spike (ms)','FontSize',15);
ylabel('STA stimulus');
plot([-lookback:0], STA);